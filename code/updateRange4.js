...
Range = function(_parentElement, _data, _filterData) {
    ...
    check_selection_nb_cuts = document.getElementById("check_selection_nb_cuts")
        .checked;
    check_selection_egs = document.getElementById("check_selection_egs")
        .checked;
    check_selection_mms = document.getElementById("check_selection_mms")
        .checked;
    check_selection_hmss = document.getElementById("check_selection_hmss")
        .checked;
    check_selection_p_votes = document.getElementById("check_selection_p_votes")
        .checked;
    ...
    this.default_var_min = d3.min(this.data, function(d) {
        return +d[col_to_filter];
    });
    this.default_var_max = d3.max(this.data, function(d) {
        return +d[col_to_filter];
    });
    this.filter_var_min = d3.min(this.filterData, function(d) {
        return +d[col_to_filter];
    });
    this.filter_var_max = d3.max(this.filterData, function(d) {
        return +d[col_to_filter];
    });
    ...
    if (graph) {
        ...
        if (path.includes("/data/VA_data/plan_metrics_ATG17.csv")) {
        ...
        if (check_selection_nb_cuts) {
            ...
            thresholdLabel_max = svg_nb_cuts
                .append("text")
                .text("Max: " + tmax_tmp);
            thresholdLabel_min = svg_nb_cuts
                .append("text")
                .text("Min: " + tmin_tmp);
        }
        if (check_selection_egs) {
            ...
            thresholdLabel_max = svg_egs
                .append("text")
                .text("Max: " + tmax_tmp + "%");
            thresholdLabel_min = svg_egs
                .append("text")
                .text("Min: " + tmin_tmp + "%");
        }
        if (check_selection_mms) {
            ...
            thresholdLabel_max = svg_mms
                .append("text")
                .text("Max: " + tmax_tmp);
            thresholdLabel_min = svg_mms
                .append("text")
                .text("Min: " + tmin_tmp);
        }
        if (check_selection_hmss) {
            ...
            thresholdLabel_max = svg_hmss
                .append("text")
                .text("Max: " + tmax_tmp);
            thresholdLabel_min = svg_hmss
                .append("text")
                .text("Min: " + tmin_tmp);
        }
        if (check_selection_p_votes) {
            ...
            tmin_tmp = (tmin_tmp * 100).toFixed(1);
            tmax_tmp = (tmax_tmp * 100).toFixed(1);
            thresholdLabel_max = svg_p_votes
                .append("text")
                .text("Max: " + tmax_tmp + "%");
            thresholdLabel_min = svg_p_votes
                .append("text")
                .text("Min: " + tmin_tmp + "%");
        }
...
Range.prototype.update_range = function() {
    ...
    filterData = filterData.filter(function(d) {
        return (
            +d[col_to_filter] <= filter_var_max &&
            +d[col_to_filter] >= filter_var_min
        );
    });
    ///////////////////////////////////////////////
...
function setThreshold1(t, eventFromUser, max_true) {
    ...
    if (max_true) {
        ...
        thresholdLabel_max.exit().remove();
    } else {
        filter_var_min = t;
        ...
        thresholdLabel_min.exit().remove();
    }
}

// listening to drag events
var drag_max1 = d3.drag().on("drag", function() {
    tx_max1 += d3.event.dx;
    if (check_selection_nb_cuts) {
        var t_max1 = xScale_nb_cuts.invert(tx_max1);
    }
    if (check_selection_p_votes) {
        var t_max1 = xScale_p_votes.invert(tx_max1);
    }
...
// listening to drag events
var drag_min1 = d3.drag().on("drag", function() {
    tx_min1 += d3.event.dx;

    if (check_selection_nb_cuts) {
        var t_min1 = xScale_nb_cuts.invert(tx_min1);
    }
    if (check_selection_p_votes) {
        var t_min1 = xScale_p_votes.invert(tx_min1);
    }
...
// listening to drag events
var drag_max2 = d3.drag().on("drag", function() {
    tx_max2 += d3.event.dx;

    if (check_selection_egs) {
        var t_max2 = xScale_egs.invert(tx_max2);
    }
    if (check_selection_mms) {
        var t_max2 = xScale_mms.invert(tx_max2);
    }
    if (check_selection_hmss) {
        var t_max2 = xScale_hmss.invert(tx_max2);
    }
...
// listening to drag events
var drag_min2 = d3.drag().on("drag", function() {
    tx_min2 += d3.event.dx;
    if (check_selection_egs) {
        var t_min2 = xScale_egs.invert(tx_min2);
    }
    if (check_selection_mms) {
        var t_min2 = xScale_mms.invert(tx_min2);
    }
    if (check_selection_hmss) {
        var t_min2 = xScale_hmss.invert(tx_min2);
    }
...
function disp_text() {
    if (document.getElementById("description").value == "1") {
        document.getElementById("message").innerHTML =
            "If we split the maps into tiny squares, the number of cut edges between the districts in a particular plan.";
    } else if (document.getElementById("description").value == "2") {
        document.getElementById("message").innerHTML =
            "How democratic is the most democratic congressional district in the state?" +
            "\n" +
            "A high percentage implies partisan imbalance.";
    } else if (document.getElementById("description").value == "3") {
        document.getElementById("message").innerHTML =
            "The difference between the average vote share of either party across all districts from the median vote share of the same party across all districts." +
            "\n" +
        ...
        document.getElementById("message").innerHTML =
            "Number of seats won by Democrats out of the total number of electoral seats." +
            "\n" +
        ...
        document.getElementById("message").innerHTML =
            "The difference between the parties' respective wasted votes, divided by the total number of votes cast in the election" +
            "\n" +
